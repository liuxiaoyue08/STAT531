...
* Write the data as $x_1,\dots,x_T$. 

* A basic estimate of the mean and standard deviation is $\hat\mu= \frac{1}{T}\sum_{t=1}^Tx_t$ and $\hat\sigma^2= \frac{1}{T-1}\sum_{t=1}^T(x_t-\hat\mu)^2$, suggesting an approximate confidence interval for $\mu$ of $\hat\mu\pm 1.96\hat\sigma/\sqrt{T}$, 

* 1955 has missing data, coded as `NA`, requiring a minor modification. So, we compute $\hat\mu_1$ and $SE_1=\hat\sigma/\sqrt{T}$ as
...
* Answer:Constant mean+variance+fixed distribution
  independence, close enough to normal for a central limit theorem to be a good approximation
  
<br>

### Question: When, in practice, is it reasonable to present this confidence interval? Is it reasonable here?
* Temperory and dependent. Answer this question later along the course
...
$$ x_t = \mu + \alpha(x_{t-1}-\mu) + \epsilon_t + \beta \epsilon_{t-1}.$$
This has a one-lag autoregressive term, $\alpha(x_{t-1}-\mu)$, and a one-lag moving average term, $\beta \epsilon_{t-1}$. It is therefore called an ARMA(1,1) model. These lags give the model some time dependence. 

    + If $\alpha=\beta=0$, we get back to the basic independent model, $x_t = \mu + \epsilon_t$.
 
    + If $\alpha=0$ we have a moving average model with one lag, MA(1).

    + If $\beta=0$, we have an autoregressive model with one lag, AR(1).

* We suppose that $\epsilon_1\dots,\epsilon_T$ is an independent, identically distributed sequence. To be concrete, let's suppose they are normally distributed with mean zero and variance $\sigma^2$.
...
* We should do **diagnostic analysis**. The first thing to do is to look at the residuals. For an ARMA model, the residual $r_t$ at time $t$ is defined to be the difference between the data, $x_t$, and its predicted value, written as $\hat x_t$. From the ARMA(1,1) definition, 
$$ x_t = \mu + \alpha(x_{t-1}-\mu) + \epsilon_t + \beta \epsilon_{t-1},$$
a simple one-step-ahead predicted value corresponding to parameter estimates $\hat\mu$ and $\hat\alpha$ could be
$$\hat x_t = \hat\mu + \hat\alpha(x_{t-1}-\hat\mu).$$
A so-called residual time series, $\{r_t\}$, is then given by
$$ r_t = x_t - \hat x_t.$$
...
However, the residuals are close to uncorrelated, as we can check by plotting their pairwise sample correlations at a range of lags. This is called the autocorrelation function, or ACF, which is computed for the residual time series $\{r_t\}$ as
$$ ACF(\ell) = \frac{1}{T-\ell}\sum_{t=1}^{T-\ell} r_{t+\ell}\, r_t.$$
...
* Let $\{y_t,t=1,\dots,T\}$ be the daily returns on a stock market index, such as the S&P 500. 
...
$$
\begin{aligned} Y_t &= \exp\{H_t/2\} \epsilon_t, \\
H_t &= \mu_h(1-\phi) + \phi H_{t-1} + Y_{t-1}\sigma_\eta\sqrt{1-\phi^2}\tanh(G_{t-1}+\nu_t)\exp\{-H_{t-1}/2\} + \omega_t,\\
G_t &= G_{t-1}+\nu_t,
\end{aligned}
$$

* $\{\epsilon_t\}$ is an iid $N(0,1)$ sequence, $\{\nu_t\}$ is an iid $N(0,\sigma_{\nu}^2)$ sequence, and $\{\omega_t\}$ is an iid $N(0,\sigma_\omega^2)$ sequence.

* $H_t$ represents the volatility at time $t$. Volatility is unobserved; we only get to observe the return, $Y_t$.

* $G_t$ is a slowly-varying process that regulates $H_t$.

*  $H_t$ has auto-regressive behavior and dependence on the previous return, $Y_{t-1}$, as well as being driven by $G_t$.
